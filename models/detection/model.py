...
class UpBlock(nn.Module):
    ...
    def forward(self, x1, x2):
        # x1 is from the deeper layer, x2 is the skip connection from the encoder
        x1 = self.up(x1)  
...
class TextDetectionModel(nn.Module):
    ...
    def forward(self, x):
        ...
        x4_att = self.att1(x5, x4)  # Gating signal: x5 (1024), Skip connection: x4 (512) -> x4_att (512 channels)
        
        # Up path with attended features
        x = self.up1(x5, x4_att)   # Input: x5 (1024), x4_att (512) -> 512 channels
        
        x3_att = self.att2(x, x3)  # Gating signal: x (512), Skip connection: x3 (256) -> x3_att (256 channels)
        x = self.up2(x, x3_att)    # Input: x (512), x3_att (256) -> 256 channels
        
        x2_att = self.att3(x, x2)  # Gating signal: x (256), Skip connection: x2 (128) -> x2_att (128 channels)
        x = self.up3(x, x2_att)    # Input: x (256), x2_att (128) -> 128 channels
        
        x1_att = self.att4(x, x1)  # Gating signal: x (128), Skip connection: x1 (64) -> x1_att (64 channels)
        x = self.up4(x, x1_att)    # Input: x (128), x1_att (64) -> 64 channels
